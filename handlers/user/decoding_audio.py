...
from telethon_api.user_bot import get_big_data
...
async def go_decoding(call: types.CallbackQuery, state: FSMContext, session_maker: sessionmaker):
    ...
    if user_info.remaining_seconds < data['duration_sec']:
        ...
        msg = await data['msg'].edit_text(
            text='<b>Спасибо! Я начал обработку файла, и это займет примерно 5-10 минут.</b>'
        )
        await asyncio.sleep(2)
        data = await state.get_data()
        try:
            if data['size'] > 20971520:
                path, duration, size = await get_big_data(call.from_user.id, msg.message_id)
                await state.update_data(path=path)
                data = await state.get_data()
                msg = await call.message.answer('<b>Загружено, идет расшифровка...</b>')
            ...
            data = await state.get_data()
            logging.error(f'Decoding error {_ex} --> go_decoding_handler')

            try:
                os.remove(data['path'])
            except Exception as _ex:
                logging.error(f'Delete error {_ex} --> go_decoding_handler --> {data["path"]}')
